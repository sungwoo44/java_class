...
Exception (예외 처리부)
*/
SELECT * FROM TBL_CUSTOM tc ;-- 익명의 일회성 프로시저
DECLARE   -- 변수선언부  
...
/*==================================================================
	...
	( 매개변수이름 [IN 또는 OUT] 데이터 타입,.....)	IN: 입력매개변수
	[IS 또는 AS]									OUT: 출력매개변수
		프로시저 변수 선언
	BEGIN
		프로시저 작업 내용 SQL들....
...
BEGIN 
   ...
   DBMS_OUTPUT.PUT_LINE(CHR(10)||'고객이름 : ' || vname);  
   DBMS_OUTPUT.PUT_LINE('고객나이 : ' || vage);
   EXCEPTION      -- 예외(오류)처리
...
BEGIN
   SELECT max(quantity)		---구매 수량 최대값
      INTO maxval 			-- 조회 결과를 (프로시저 일반)-> 변수'maxval'에 저장
   FROM tbl_buy;  			 

   SELECT customid
      INTO cid				--조회 결과를 (프로시저 일반 변수0->'cid'에 저장
   FROM tbl_buy
...
DECLARE
	...
	vage tbl_custom.age %TYPE;
BEGIN
	max_custom(vname,vage);			--- 프로시저 실행 결과값을, vname, vage 변수에 저장

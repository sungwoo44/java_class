...
CREATE TABLE tbl_member (
	...
	join_date DATE default sysdate  	-- 가입날짜 , 날짜형식 : 날짜 패턴 문자열이 자동으로 날짜형식 변환(insert 할때)
...
-- `이메일이 naver 인 회원`
	SELECT * FROM TBL_MEMBER tm WHERE EMAIL  LIKE '%naver%';

-- 6) 여러가지 값으로 조회 : `이름이 김모모 또는 박나연 또는 강나현`
 	SELECT name FROM TBL_MEMBER tm WHERE tm.MNO in(1,2,6);
 	SELECT name FROM TBL_MEMBER tm WHERE name='김모모' OR name='박나연' OR name='강나현' ;

/* `제약 조건 constraints`
   ...
   4) FOREIGN KEY (외래키): 다른 테이블의 값을 참조하기 위해 사용
*/   


-- 1) 학생 테이블
CREATE TABLE tbl_student (
	stuno char(7) PRIMARY KEY,   -- PK 설정방법 1) 컬럼레벨 
...
-- 참조관계일때 테이블 삭제 
-- 참조관계일 경우 부모테이블(tbl_student) drop 불가 so, 자식테이블(tbl_score) 부터 삭제후 부모테이블 삭제가능
DROP TABLE TBL_STUDENT ;   -- 참조되는 동안에는 삭제 안됨. 
...
INSERT INTO tbl_student(stuno,name,age,address)
VALUES ('2021001','김땡땡',18,'성북구');				--오류 stuno 가 중복됨
...
INSERT INTO tbl_score(stuno,subject,jumsu,teacher,term)
VALUES ('2021002','국어',89,'이나연','2022_1'); 				-- 오류 
INSERT INTO tbl_score(stuno,subject,jumsu,teacher,term)
VALUES (null,'국어',89,'이나연','2022_1'); 				-- 오류  기본키를 구성하는 컬럼은 NULL 불가       
...
- 성적테이블에 '과목명'을 학생 1명이 동일한 과목을 수강한다면(예를 들면 1학년 국어 ,  2학년 국어, 3학년 국어 )
  새로운 기본키를 만들어야 합니다. 일련번호 또는 성적코드 등.
  1) 새로운 기본키를 만들어야 합니다. 일련번호 또는 성적코드
  2) '과목'테이블 - 과목코드,과목명(중복허용),담당교사과목명(중복허용)
  
  ★★★★☆정규화 
  	테이블은 최소로 데이터가 중복되도록 구성한다. 데이터 중복은 불일치를 발생시킨다. 
  	ex) 정규화가 잘되어있다. --> 테이블 구성을 잘했다.

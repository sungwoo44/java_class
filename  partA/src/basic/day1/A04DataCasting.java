...
public class A04DataCasting {
	...
	public static void main(String[] args) {
		...
		avgDouble = (double)sum/count;		//2))	//sum을 강제로 변환(캐스팅)
 		System.out.println("실수형식으로 변환 후 평균 :" + avgDouble);


		System.out.println("자동 캐스팅도 있다.");
		double dnum =34; //  dnum은,  double형식, 34는 정수형식, 정수를 실수로 변환은 자동
		//int inum = 3.1415; //  실수를 정수로 변화은 강제 
		int inum = (int)3.1415;

		System.out.println("자동 캐스팅된 34 ="+dnum);
		System.out.println("강제 캐스팅된 34 ="+inum);
		inum = (int)3.6789;
		System.out.println("강제 캐스팅된 34 ="+inum);
		 ...
		 *  1	    2      4      8       4       8		바이트 
		 * 
		 * char 문자형을 코드값 구하기 위해서 정수로 캐스팅할 때에는 0~65535를 
		 * 충분히 저장할수 있는  int   정수형으로 캐스팅합니다.
		 * 
		 */
